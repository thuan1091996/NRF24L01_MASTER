...
void main(void)
{
    ...
    Button_Init();                              //Pull-up button    (PF4)
    //----------------Interrupt Enable----------------------------
    IntMasterEnable();
    //----------------Infinite Loop-------------------------------
    while(true)
    {
        unsigned char ui8Data;
        unsigned int ui32Button;
        ui32Button=GPIOPinRead(BUTTON_BASE, BUTTON_PIN); //Read button
        if(ui32Button==0)                                //If pressed
        {
            ui8Data=0x01;
            nrf24l01_write_tx_payload(&ui8Data, 1, true);//If button pressed, send 0x01
...
void NRF24L01_Init(){
    ...
    nrf24l01_initialize_debug(false, 1, true); //TX,1 Byte, Enhanced ShockBurst
...
void Monitor(void)
{
        if(Tick_monitor>=100000) //Check system each 0.1s
        {
           ...
           if(Error>=5)    { State=WARNING; LEDs_DATA_R=0;}
           if(Error>=10)   { State=FAIL;    LEDs_DATA_R=0;}
           if(State==NORMAL)       LEDs_DATA_R^=GREEN;
